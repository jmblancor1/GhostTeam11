...
function avPag(){
    ...
    if(curPageMaxY - curY >= viewportHeight){ 
        if(curPageMaxY - (curY + viewportHeight) >= viewportHeight){
            ...
            const remainingScroll = curPageMaxY - curY;
            if (remainingScroll > 0) {
                const scrollableElement = cy.get('body');
                scrollableElement.then($el => {
                    const elementIsScrollable = $el[0].scrollHeight > $el[0].clientHeight;
                    if (elementIsScrollable) {
                        curY = curPageMaxY - viewportHeight;
                        cy.scrollTo(curX, curY);
                        info += "Page limit reached! ";
                    } else {
                        info += "Element is not scrollable. Unable to scroll down.";
                    }
                });
            } else {
        ...
        if(viewportHeight > curY){
            ...
            scrollableElement.then($el => {
                ...
                if (elementIsScrollable) {
                    ...
                    info += "Element is not scrollable. Unable to scroll up.";
                }
            });
...
function horizontalScrollFw(){
    ...
    if(curPageMaxX - curX >= viewportWidth){ 
        ...
        // En el eje X no detecta scrollTo 
        // else{
...
function navBack(){
    cy.url().then((path)=>{
        let info = ""
        cy.task("logCommand", { funtype: `url: ${url}, path: ${path}`, info: `url: ${url}, path: ${path}`})
        if(url!==path){
            if (path !== 'http://localhost:2368/ghost/#/signin') {
                cy.go(-1)
...
function fillInput() { // Or fill form
    cy.window().then((win) => {
        let $inputs = win.document.querySelectorAll("input, textarea"); // Include textarea
        let info = "";
        if ($inputs.length > 0) {
            var inp = $inputs.item(getRandomInt(0, $inputs.length));
            if (!Cypress.dom.isHidden(inp)) {
                focused = true;
                if (inp.tagName.toLowerCase() === "textarea") { // Check if it's a textarea
                    let type = faker.lorem.words(); // Example text generation
                    cy.wrap(inp).type(type);
                    info = `Textarea ${inp.id} was filled with ${type}`;
        ...
        } else info = "INVALID. There are no input or textarea elements in the current page";
        console.log(info);
        cy.task("logCommand", {
            funtype: "Action: click anchor",
            info: info
        });
    });
}
...
describe( `${appName} under smarter monkeys`, function() {
    //Listeners
    before(() => {
        cy.visit('https://ghost-v5-8wq2.onrender.com/ghost/#/signin')
        cy.title('eq','Ghost Admin - Ghost')
        cy.wait(3000)
        login('df.guerrero@uniandes.edu.co', 'qwerty1234');
    ...
    it(`visits ${appName} and survives smarter monkeys`, function() {
        ...
        if(pcg < 100) {
            ...
            pct_keys *= factor;
            pct_pgnav *= factor;
            pct_selectors *= factor;
            pct_spkeys *= factor;
            pct_actions *= factor;
            pct_browserChaos *= factor;
        }
    
        pending_events[0] = Math.ceil(events * (pct_clicks / 100));
        pending_events[1] = Math.ceil(events * (pct_scrolls / 100));
        pending_events[2] = Math.ceil(events * (pct_selectors / 100));
        pending_events[3] = Math.ceil(events * (pct_keys / 100));
        pending_events[4] = Math.ceil(events * (pct_spkeys / 100));
        pending_events[5] = Math.ceil(events * (pct_pgnav / 100));
        pending_events[6] = Math.ceil(events * (pct_browserChaos / 100));
        pending_events[7] = Math.ceil(events * (pct_actions / 100));
