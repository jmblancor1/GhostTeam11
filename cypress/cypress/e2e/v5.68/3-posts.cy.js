...
describe('Create Post draft in Ghost', () => {
    ...
    it('debería crear y luego editar un borrador', () => {
        ...
        cy.screenshot('/v5.14/caso11/1-creating-draft');
        ...
        cy.screenshot('/v5.14/caso11/2-ValidationSucessMesaje');
        //cy.xpath("//button/span[text()=' Editor']").click();
        cy.wait(500);
        //y.xpath("//a/span[text()='Posts']").click();
        cy.visit('/#/posts?type=draft');
        cy.wait(500)
        cy.screenshot('/v5.14/caso11/3-ValidationItemCreated');
        ...
        cy.screenshot('/v5.14/caso11/4-ValidationItemForEditing');
        cy.get('textarea[placeholder="Post title"]').first().clear().type('deporte');
        cy.wait(500);
        cy.screenshot('/v5.14/caso11/5-editing-draft-tittle.png');
        //click boton publicar
        cy.xpath("//span[normalize-space()=\'Publish\']").click();
        //se valida la creacion esitoso del post
        cy.contains('Ready, set, publish. Share it with the world.').should('be.visible');
        cy.screenshot('/v5.14/caso11/6-ValidationSucessMesajeEdition');
        // se valida que la actualizacion del titulo estuvo bien
        cy.visit('/#/posts?type=draft');
        cy.contains('deporte').should('exist');
        cy.wait(500);
        cy.screenshot('/v5.14/caso11/7-editing-draft-ValidationItemEdited.png');
    ...
    it('debería crear un draft post sin descripción', () => {
        ...
        cy.screenshot('/v5.14/caso12/1-creating-draft');
        ...
        cy.screenshot('/v5.14/caso12/2-ValidationSucessMesaje');
        cy.visit('/#/posts?type=draft');
        cy.contains('cocina').should('exist');
        cy.screenshot('/v5.14/caso12/3-ValidationItemCreated.png');
    ...
    it('debería eliminar un draft post creado', () => {
        ...
        //click boton publicar
        cy.wait(500);
        cy.screenshot('/v5.14/caso13/1-creating-draft.png');
        cy.xpath('//span[normalize-space()=\'Publish\']').click();
        //se valida la creacion esitoso del post
        cy.contains('Ready, set, publish. Share it with the world.').should('be.visible');
        cy.screenshot('/v5.14/caso13/2-ValidationSucessMesaje');
        //click botón Editing
        cy.xpath("//button/span[text()=' Editor']").click();
        cy.wait(500);
        cy.screenshot('/v5.14/caso13/3-ValidationEditorButton');    
        //Accediendo al botón Setings
        cy.get('.settings-menu-toggle').click({force: true});
        cy.screenshot('/v5.14/caso13/4-ValidationSettingsButton');    
        ...
        cy.wait(500);
        cy.screenshot('/v5.14/caso13/5-validationMessageDelete');        
        //click botón eliminar del Modal
        cy.xpath("//button/span[text()='Delete']").click();
        cy.screenshot('/v5.14/caso13/6-ValidationDeleteButton');    
        cy.visit('/#/editor/post');
        cy.contains('New post for deleting').should('not.exist');
        cy.wait(500);
        cy.screenshot('/v5.14/caso13/7-ValidationItemDeleted');
    ...
    it('Cancelar la eliminacion de un draft post creado', () => {
        ...
        //click boton publicar
        cy.wait(500);
        cy.screenshot('/v5.14/caso14/1-creating-draft.png');
        cy.xpath('//span[normalize-space()=\'Publish\']').click();
        //se valida la creacion esitoso del post
        cy.contains('Ready, set, publish. Share it with the world.').should('be.visible');
        cy.screenshot('/v5.14/caso14/2-ValidationSucessMesaje');
        //click botón Editing
        cy.xpath("//button/span[text()=' Editor']").click();
        cy.wait(500);
        cy.screenshot('/v5.14/caso14/3-validationEditorButton');    
        //Accediendo al botón Setings
        cy.get('.settings-menu-toggle').click({force: true});
        //Accediendo al botón Delete
        cy.screenshot('/v5.14/caso14/4-validationSettingsButton');   
        cy.xpath("//button/span[text()=' Delete ']").click({force: true});
        cy.screenshot('/v5.14/caso14/5-validationDeleteButton');   
        cy.contains('Are you sure you want to delete this post?').should('be.visible');
        cy.wait(500);
        cy.screenshot('/v5.14/caso14/6-validationMessageDelete');
           
        //click botón cancelar del Modal
        cy.xpath("//button/span[text()='Cancel']").click();
        cy.screenshot('/v5.14/caso14/7-validationCancelButton');
        cy.xpath("//a/span[text()='Posts']").click();
        cy.contains('New post for canceling').should('exist');
        cy.wait(500);
        cy.screenshot('/v5.14/caso14/8-ValidationItemCanceled');
    });
    ...
    it('Crear Draft Post y publicarlo y verificarlo en posts Publicados', () => {
        ...
        //click boton publicar
        cy.screenshot('/v5.14/caso15/1-creating-draft');
        cy.xpath("//button/span[text()='Publish']").click();
        //se valida la creacion esitoso del post
        cy.contains('Ready, set, publish. Share it with the world.').should('be.visible');
        cy.screenshot('/v5.14/caso15/2-ValidationSucessMessaje');
        //cy.xpath("//button/span[text()=' Editor']").click();
        cy.wait(500);
        //boton publicar
        cy.get(".gh-publish-cta").click();
        cy.screenshot('/v5.14/caso15/3-ValidationSucessContinue');
        cy.xpath("//button/span[text()='Publish post, right now']").click();
        cy.screenshot('/v5.14/caso15/4-ValidationConfirmationPublish');
        //mesaje de validacion de la publicacion
        cy.contains('Boom. It’s out there.').should('be.visible');
        cy.screenshot('/v5.14/caso15/5-ValidationSucessMessajePublish');
        cy.visit('/#/posts?type=published');
        cy.wait(500);
        cy.screenshot('/v5.14/caso15/6-ValidationItemPublished');
