beforeEach(() => {
  ...
  describe("Login y escenarios para tags", () => {
    ...
    it("Debería crear un tag y eliminar exitosamente", () => {
        ...
        cy.screenshot('/v3/caso20/6-type-tag-description');
        cy.wait(1000);
        cy.get("span").contains("Save").click();
        cy.screenshot('/v3/caso20/7-click-save');
        cy.wait(1000);
        cy.visit("/#/tags");
        cy.screenshot('/v3/caso20/8-visit-tags');
        cy.wait(2000);
        cy.get('.gh-tags-list-item')
          .contains('h3', 'Nuevo Tag para eliminar del equipo #11')
          .click();
        cy.screenshot('/v3/caso20/9-click-tag-created');
        cy.wait(1000);
        cy.get("span").contains("Delete tag").click();
        cy.screenshot('/v3/caso20/10-click-delete-tag');
        cy.get('.modal-content').should('be.visible');
        cy.screenshot('/v3/caso20/11-click-show-modal');
        cy.wait(3000);
        cy.get('.modal-footer').find('button').contains('Delete').click();
        cy.screenshot('/v3/caso20/12-click-delete-tag-modal');
        cy.wait(5000);
      });
  });